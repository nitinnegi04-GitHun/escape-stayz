...
export const HotelGallery: React.FC<HotelGalleryProps> = ({ images, hotelName, heroImage }) => {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-4 grid-rows-2 gap-2 h-[80vh] md:h-[60vh] overflow-hidden relative">
                {/* Main Hero Image */}
                <div
                ...
                >
                    ...
                    {mainAsset.tag && (
                        <div className="absolute top-4 left-4 bg-forest/90 backdrop-blur-md text-white px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg z-[10]">
                            {mainAsset.tag}
                        </div>
                    )}
                </div>
                ...
                <button
                    onClick={() => openGallery(0)}
                    className="absolute bottom-6 right-6 bg-white border border-charcoal/10 px-5 py-2 rounded-lg shadow-lg flex items-center gap-2 hover:scale-105 transition-transform"
                >
